...
type Rol = { id_rol: number; nombre_rol: string; salario: number; create_at: string; update_at: string };
type Form = { nombre_rol: string; salario: string };
const EMPTY: Form = { nombre_rol: '', salario: '' };
...
export default function RolesPage() {
  ...
  const openEdit = (r: Rol) => {
    setF({ nombre_rol: r.nombre_rol, salario: String(r.salario) });
    setModal({ mode: 'edit', id: r.id_rol });
  };

  const save = async () => {
    if (!f.nombre_rol.trim() || !f.salario) { setError('Nombre y salario son obligatorios.'); return; }
    setSaving(true); setError('');
    try {
      const url    = modal?.mode === 'edit' ? `/api/roles/${modal.id}` : '/api/roles';
      const method = modal?.mode === 'edit' ? 'PUT' : 'POST';
      const res    = await fetch(url, { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ nombre_rol: f.nombre_rol.trim(), salario: parseInt(f.salario, 10) }) });
      const j      = await res.json();
  ...
  const del = async (id: number) => {
    ...
    try {
      ...
      const j   = await res.json();
      if (j.success) setData(p => p.filter(r => r.id_rol !== id)); else setError(j.message);
    } catch { setError('Error de red.'); }
  };
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg, color: C.text, fontFamily: "'Inter','Segoe UI',sans-serif" }}>
      ...
      <nav style={{ background: C.sidebar, borderBottom: `1px solid ${C.borderLight}`, padding: '0 24px', height: 56, display: 'flex', alignItems: 'center', gap: 6, position: 'sticky', top: 0, zIndex: 50, overflowX: 'auto' }}>
        ...
        {MODULES.map(m => (
          <Link key={m.href} href={m.href} style={{ color: m.href === '/admin/roles' ? '#60a5fa' : '#64748b', textDecoration: 'none', fontSize: 11, fontWeight: 600, padding: '5px 10px', borderRadius: 7, background: m.href === '/admin/roles' ? 'rgba(0,112,243,.15)' : 'none', border: m.href === '/admin/roles' ? '1px solid rgba(0,112,243,.3)' : '1px solid transparent', whiteSpace: 'nowrap', flexShrink: 0 }}>{m.label}</Link>
        ))}
      ...
      <main style={{ padding: '32px 40px', maxWidth: 1000, margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          <div>
            ...
            <h1 style={{ margin: '4px 0 0', fontSize: 24, fontWeight: 900 }}>🛡️ Roles</h1>
          </div>
          <button onClick={openNew} style={{ background: 'linear-gradient(135deg,#003066,#1d4ed8)', color: '#fff', border: 'none', padding: '11px 22px', borderRadius: 10, fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>＋ Nuevo Rol</button>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(160px,1fr))', gap: 14, marginBottom: 24 }}>
          {[
            { l: 'Total roles', v: data.length },
            { l: 'Salario promedio', v: data.length ? `Bs ${Math.round(data.reduce((a,b) => a + b.salario, 0) / data.length).toLocaleString()}` : '—' },
            { l: 'Salario máx.', v: data.length ? `Bs ${Math.max(...data.map(r => r.salario)).toLocaleString()}` : '—' },
          ].map(({ l, v }) => (
            <div key={l} style={{ background: C.bgCard, border: `1px solid ${C.border}`, borderRadius: 12, padding: '16px 20px' }}>
              ...
              <p style={{ margin: '6px 0 0', color: C.text, fontSize: 26, fontWeight: 900 }}>{v}</p>
            </div>
          ))}
        </div>
        ...
        <div style={{ background: C.bgCard, border: `1px solid ${C.border}`, borderRadius: 16, overflow: 'hidden' }}>
          <div style={{ padding: '14px 20px', borderBottom: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', gap: 10 }}>
            <span>🔍</span>
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar por nombre de rol..." style={{ background: 'none', border: 'none', color: C.text, fontSize: 14, outline: 'none', flex: 1 }} />
          </div>
          {loading ? (
            <div style={{ padding: 40, textAlign: 'center', color: C.textMuted }}>Cargando roles...</div>
          ) : (
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr>{['ID', 'Nombre del Rol', 'Salario (Bs)', 'Creado', 'Acciones'].map((h, i) => (
                ...
                <tbody>
                  {filtered.map(r => (
                    <tr key={r.id_rol} style={{ borderBottom: `1px solid ${C.borderLight}` }}>
                      <td style={{ padding: '13px 16px', color: C.blueLight, fontWeight: 700, fontSize: 13 }}>#{r.id_rol}</td>
                      <td style={{ padding: '13px 16px', color: '#e2e8f0', fontWeight: 600, fontSize: 13 }}>{r.nombre_rol}</td>
                      <td style={{ padding: '13px 16px', color: C.green, fontWeight: 700, fontSize: 14 }}>Bs {r.salario.toLocaleString()}</td>
                      <td style={{ padding: '13px 16px', color: '#64748b', fontSize: 12 }}>{new Date(r.create_at).toLocaleDateString('es-BO')}</td>
                      <td style={{ padding: '13px 16px' }}>
                        <div style={{ display: 'flex', gap: 8 }}>
                          <button onClick={() => openEdit(r)} style={{ background: 'rgba(148,163,184,.08)', border: '1px solid #1e3a5f', color: '#94a3b8', padding: '6px 12px', borderRadius: 7, cursor: 'pointer', fontSize: 13 }}>✏️</button>
                          <button onClick={() => del(r.id_rol)} style={{ background: 'rgba(239,68,68,.1)', border: '1px solid rgba(239,68,68,.3)', color: '#f87171', padding: '6px 12px', borderRadius: 7, cursor: 'pointer', fontSize: 13 }}>🗑️</button>
                        </div>
                      </td>
                    </tr>
                  ))}
              ...
              {filtered.length === 0 && !loading && (
                <div style={{ padding: 40, textAlign: 'center', color: C.textMuted }}>
                  {data.length === 0 ? 'No hay roles registrados.' : 'Sin resultados.'}
                </div>
              )}
            </div>
          )}
      ...
      {modal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.65)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, backdropFilter: 'blur(4px)' }}>
          <div style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 16, padding: 32, width: 'min(440px,95vw)', boxShadow: '0 30px 80px rgba(0,0,0,.5)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
              <h3 style={{ margin: 0, fontSize: 18, fontWeight: 800 }}>{modal.mode === 'new' ? 'Nuevo Rol' : 'Editar Rol'}</h3>
            ...
            <Fld label="Nombre del rol">
              <Inp required value={f.nombre_rol} onChange={e => setF(p => ({ ...p, nombre_rol: e.target.value }))} placeholder="Ej: Supervisor" />
            </Fld>
            <Fld label="Salario (Bs)">
              <Inp required type="number" min="0" value={f.salario} onChange={e => setF(p => ({ ...p, salario: e.target.value }))} placeholder="3500" />
            </Fld>
