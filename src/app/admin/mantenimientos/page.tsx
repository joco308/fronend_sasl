...
type Mant = { id_mantenimiento: number; fecha_mantenimiento: string; descripcion: string | null; costo: number; create_at: string };
type Form = { fecha_mantenimiento: string; descripcion: string; costo: string };
const EMPTY: Form = { fecha_mantenimiento: '', descripcion: '', costo: '' };
...
export default function MantenimientosPage() {
  ...
  const [data, setData]     = useState<Mant[]>([]);
  ...
  const filtered = data.filter(m =>
    ...
    m.fecha_mantenimiento.includes(search)
  );

  const totalCosto = data.reduce((a, b) => a + Number(b.costo), 0);

  const openNew  = () => { setF(EMPTY); setModal({ mode: 'new' }); };
  const openEdit = (m: Mant) => {
    setF({ fecha_mantenimiento: m.fecha_mantenimiento.substring(0, 10), descripcion: m.descripcion ?? '', costo: String(m.costo) });
    setModal({ mode: 'edit', id: m.id_mantenimiento });
  };

  const save = async () => {
    if (!f.fecha_mantenimiento || !f.costo) { setError('Fecha y costo son obligatorios.'); return; }
    setSaving(true); setError('');
    try {
      const url    = modal?.mode === 'edit' ? `/api/mantenimientos/${modal.id}` : '/api/mantenimientos';
      const method = modal?.mode === 'edit' ? 'PUT' : 'POST';
      const res    = await fetch(url, { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ fecha_mantenimiento: f.fecha_mantenimiento, descripcion: f.descripcion.trim() || null, costo: parseFloat(f.costo) }) });
      const j      = await res.json();
  ...
  const del = async (id: number) => {
    ...
    try {
      ...
      const j   = await res.json();
      if (j.success) setData(p => p.filter(m => m.id_mantenimiento !== id)); else setError(j.message);
    } catch { setError('Error de red.'); }
  };
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg, color: C.text, fontFamily: "'Inter','Segoe UI',sans-serif" }}>
      ...
      <nav style={{ background: C.sidebar, borderBottom: `1px solid ${C.borderLight}`, padding: '0 24px', height: 56, display: 'flex', alignItems: 'center', gap: 6, position: 'sticky', top: 0, zIndex: 50, overflowX: 'auto' }}>
        ...
        {MODULES.map(m => (
          <Link key={m.href} href={m.href} style={{ color: m.href === '/admin/mantenimientos' ? '#60a5fa' : '#64748b', textDecoration: 'none', fontSize: 11, fontWeight: 600, padding: '5px 10px', borderRadius: 7, background: m.href === '/admin/mantenimientos' ? 'rgba(0,112,243,.15)' : 'none', border: m.href === '/admin/mantenimientos' ? '1px solid rgba(0,112,243,.3)' : '1px solid transparent', whiteSpace: 'nowrap', flexShrink: 0 }}>{m.label}</Link>
        ))}
      ...
      <main style={{ padding: '32px 40px', maxWidth: 1100, margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          <div>
            ...
            <h1 style={{ margin: '4px 0 0', fontSize: 24, fontWeight: 900 }}>🔧 Mantenimientos</h1>
          </div>
          <button onClick={openNew} style={{ background: 'linear-gradient(135deg,#003066,#1d4ed8)', color: '#fff', border: 'none', padding: '11px 22px', borderRadius: 10, fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>＋ Nuevo Mantenimiento</button>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(160px,1fr))', gap: 14, marginBottom: 24 }}>
          {[
            { l: 'Total registros', v: data.length },
            { l: 'Costo total', v: `Bs ${totalCosto.toLocaleString('es-BO', { minimumFractionDigits: 2 })}` },
          ].map(({ l, v }) => (
            <div key={l} style={{ background: C.bgCard, border: `1px solid ${C.border}`, borderRadius: 12, padding: '16px 20px' }}>
              ...
              <p style={{ margin: '6px 0 0', color: l === 'Costo total' ? C.yellow : C.text, fontSize: 22, fontWeight: 900 }}>{v}</p>
            </div>
          ))}
        </div>
        ...
        <div style={{ background: C.bgCard, border: `1px solid ${C.border}`, borderRadius: 16, overflow: 'hidden' }}>
          <div style={{ padding: '14px 20px', borderBottom: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', gap: 10 }}>
            <span>🔍</span>
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar por descripción o fecha..." style={{ background: 'none', border: 'none', color: C.text, fontSize: 14, outline: 'none', flex: 1 }} />
          </div>
          {loading ? (
            <div style={{ padding: 40, textAlign: 'center', color: C.textMuted }}>Cargando mantenimientos...</div>
          ) : (
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr>{['ID', 'Fecha', 'Descripción', 'Costo (Bs)', 'Registrado', 'Acciones'].map((h, i) => (
                ...
                <tbody>
                  {filtered.map(m => (
                    <tr key={m.id_mantenimiento} style={{ borderBottom: `1px solid ${C.borderLight}` }}>
                      <td style={{ padding: '13px 16px', color: C.blueLight, fontWeight: 700, fontSize: 13 }}>#{m.id_mantenimiento}</td>
                      <td style={{ padding: '13px 16px', color: '#e2e8f0', fontWeight: 600, fontSize: 13 }}>{new Date(m.fecha_mantenimiento).toLocaleDateString('es-BO')}</td>
                      <td style={{ padding: '13px 16px', color: '#cbd5e1', fontSize: 13, maxWidth: 280 }}>{m.descripcion ?? '—'}</td>
                      <td style={{ padding: '13px 16px', color: C.yellow, fontWeight: 700, fontSize: 14 }}>Bs {Number(m.costo).toLocaleString('es-BO', { minimumFractionDigits: 2 })}</td>
                      <td style={{ padding: '13px 16px', color: '#64748b', fontSize: 12 }}>{new Date(m.create_at).toLocaleDateString('es-BO')}</td>
                      <td style={{ padding: '13px 16px' }}>
                        <div style={{ display: 'flex', gap: 8 }}>
                          ...
                          <button onClick={() => del(m.id_mantenimiento)} style={{ background: 'rgba(239,68,68,.1)', border: '1px solid rgba(239,68,68,.3)', color: '#f87171', padding: '6px 12px', borderRadius: 7, cursor: 'pointer', fontSize: 13 }}>🗑️</button>
                        </div>
                      </td>
                    </tr>
                  ))}
              ...
              {filtered.length === 0 && !loading && <div style={{ padding: 40, textAlign: 'center', color: C.textMuted }}>{data.length === 0 ? 'No hay mantenimientos registrados.' : 'Sin resultados.'}</div>}
            </div>
          )}
        </div>
      </main>
      ...
      {modal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.65)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, backdropFilter: 'blur(4px)' }}>
          <div style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 16, padding: 32, width: 'min(500px,95vw)', boxShadow: '0 30px 80px rgba(0,0,0,.5)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
              <h3 style={{ margin: 0, fontSize: 18, fontWeight: 800 }}>{modal.mode === 'new' ? 'Nuevo Mantenimiento' : 'Editar Mantenimiento'}</h3>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              <Fld label="Fecha de mantenimiento">
                <Inp required type="date" value={f.fecha_mantenimiento} onChange={e => setF(p => ({ ...p, fecha_mantenimiento: e.target.value }))} />
              </Fld>
              <Fld label="Costo (Bs)">
            ...
            <Fld label="Descripción (opcional)">
              <textarea value={f.descripcion} onChange={e => setF(p => ({ ...p, descripcion: e.target.value }))} rows={3} placeholder="Detalle del mantenimiento..." style={{ width: '100%', background: '#1e293b', border: '1px solid #334155', borderRadius: 8, padding: '10px 14px', color: '#e2e8f0', fontSize: 14, outline: 'none', resize: 'vertical', boxSizing: 'border-box' }} />
            </Fld>
